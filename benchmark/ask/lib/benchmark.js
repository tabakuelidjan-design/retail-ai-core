 ...
 * @returns {Promise<{ runs: object[][], provider: object, datasetFile: string, repeatMetadata: object[] }>} `runs[k]` = the case scores of repetition k+1
 */
export async function runBenchmark({ cases, createProvider, repeats = 1, only = null, timeoutMs = 60_000, onCase = null }) {
  ...
  const runs = []; const repeatMetadata = []; let provider = null; let datasetFile = null;
  for (let repeat = 1; repeat <= repeats; repeat += 1) {
    ...
    runs.push(scores); repeatMetadata.push({ repeat, ...(p.metadata?.() ?? {}) });   // each repetition has its own provider instance, hence its own usage totals and call counts
  }
  return { runs, provider, datasetFile, repeatMetadata };
}
