...
**Status: Phase 2A — Sales & Profit engine (under review).** Read-only Shopify → Supabase sync (Phase 1) plus a deterministic sales/profit metric layer, product signals and data-quality flags (Phase 2A). No decision engine, LLM recommendations, or dashboard yet. See [`docs/architecture/sales-profit-engine.md`](docs/architecture/sales-profit-engine.md).
