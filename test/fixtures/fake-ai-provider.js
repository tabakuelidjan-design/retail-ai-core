...
export function honestExplanation(facts) {
  ...
  if (dpct) { const v = Number((dpct.value * 100).toFixed(2)); claims.push({ text: `Le chiffre d'affaires net évolue de ${fmt(v)} % par rapport à la période de référence.`, factRefs: [dpct.ref], quantities: [q('percent', v, dpct.ref)] }); }
  ...
  if (top) claims.push({ text: `Le premier produit est « ${top.value} ».`, factRefs: [top.ref] });
...
export function createFakeProvider({ planner = defaultPlanner, explainMode = 'honest', planError = null, explainError = null } = {}) {
  ...
  return {
    ...
    async explain(input) {
      ...
      return explanationFor(explainMode, input.facts);
    },
  };
}
