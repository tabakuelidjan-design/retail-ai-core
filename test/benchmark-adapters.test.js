...
test('usage and cost: tokens (input incl. cache, output incl. reasoning/thinking, cached, cache-write, reasoning) are read per provider and priced from the config; drainUsage returns the calls since the last drain', async () => {
  const cases = { openai: { usage: { input: 1000, output: 300, cached: 200, cacheWrite: 0, reasoning: 100 }, expect: { inputTokens: 1000, outputTokens: 300, cachedTokens: 200, cacheWriteTokens: 0, reasoningTokens: 100, cost: (800 * 2 + 200 * 0.5 + 300 * 10) / 1e6 } },
