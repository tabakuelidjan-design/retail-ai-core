...
test('ISOLATION: only company.js may touch the network; credit-risk is imported by nothing; no provider names in the core', () => {
  ...
  assert.deepEqual(finFiles.filter((f) => /\bfetch\(|fetchImpl\(/.test(code(f))).sort(), ['company-search.js', 'company.js', 'inbox.js']); // the provider adapters and the private attachment-storage adapter, nothing else
